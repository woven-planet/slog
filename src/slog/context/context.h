...
#include "src/slog/context/notification_queue.h"
...
class SlogContext {
 public:
  static SLOG_INLINE SlogContext& getInstance() {
  ...
  SLOG_INLINE void notifySubscribers(SlogRecord&& record) {
    ...
    CHECK_NOTNULL(async_notification_queue_.get())->add(std::move(record));
  }

  SLOG_INLINE void waitAsyncSubscribers() {
    std::shared_lock<std::shared_timed_mutex> lock(
        async_notification_queue_mutex_);
    CHECK_NOTNULL(async_notification_queue_.get())->waitRecordsFlush();
  }
  ...
  SLOG_INLINE const SlogCallSite& getCallSite(size_t call_site_id) {
    std::shared_lock<std::shared_timed_mutex> lock(call_sites_mutex_);
    CHECK_LT(call_site_id, call_sites_.size());
 ...
 private:
  ...
  SLOG_INLINE void echoToGlog(const SlogRecord& record) {
    if (record.severity() == google::FATAL || record.isNoisy()) {
      SlogPrinter::emitGlog(record, getCallSite(record.call_site_id()));
    }
  }
