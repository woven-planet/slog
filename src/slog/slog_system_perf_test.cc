...
// command:
/*
 $> ./avctl build //src/slog:slog_system_perf_test && \
    ./bazel-bin/src/slog/slog_system_perf_test
*/
//
// 2020-11-11
//
...
#include "src/avs/libs/resource_management/cpu_allocation.h"
#include "src/common/system/runtime_metrics.h"
...
void test_new_process(uint64_t id, avsoftware::avs::CpuAllocator* cpu_alloc) {
  pid_t pid = fork();
...
using namespace avsoftware;

int main() {
  avsoftware::avs::CpusetMap cpuset_map;
  cpuset_map["non_avs"] = "2-11";
  cpuset_map[kSlogCpuset] = "0-1";
  auto cpu_allocator = avsoftware::avs::createCpuAllocator(cpuset_map);
