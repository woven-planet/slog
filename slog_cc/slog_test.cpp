...
using slog::kSlogTagKeyScopeDepth;
using slog::kSlogTagKeyScopeId;
using slog::SlogCallSite;
using slog::SlogContext;
using slog::SlogPrinter;
using slog::SlogRecord;
using slog::SlogScope;
using slog::SlogSubscriber;
using slog::SlogTag;
using slog::SlogTagValueType;
using slog::SlogTagVerbosity;
...
TEST_F(SlogTest, severity) {
  ...
  waitSlog();
  ASSERT_EQ(1, slog_records_.size());
  EXPECT_EQ(slog::INFO, slog_records_.back().severity());

  SLOG(WARNING) << "warning";
  waitSlog();
  ASSERT_EQ(2, slog_records_.size());
  EXPECT_EQ(slog::WARNING, slog_records_.back().severity());

  SLOG(ERROR) << "error";
  waitSlog();
  ASSERT_EQ(3, slog_records_.size());
  EXPECT_EQ(slog::ERROR, slog_records_.back().severity());
  ...
  // silent SLOG(FATAL) should trigger LOG(FATAL) as well, while non-fatal
